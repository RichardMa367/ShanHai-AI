...
const getAttractionImage = (name: string, index: number): string => {
  ...
  // 返回一个纯色占位图(避免跨域问题)
  const colors = [
    { start: '#e3e9f9', end: '#b8caf0' },
    { start: '#d6f9e9', end: '#a8f0ce' },
    { start: '#e3e9f9', end: '#d6f9e9' },
    { start: '#b8caf0', end: '#a8f0ce' },
    { start: '#e3e9f9', end: '#d6f9e9' }
  ]
...
const exportAsImage = async () => {
  try {
    ...
    exportContainer.style.backgroundColor = '#e3e9f9'
    ...
    cards.forEach((card) => {
      ...
      try {
        ...
        cardEl.style.setProperty('border', '1px solid #e2e8f0')
    ...
    cardHeads.forEach((head) => {
      ...
      try {
        ...
        headEl.style.setProperty('color', '#1a2332')
        headEl.style.setProperty('border-bottom', '1px solid #e2e8f0')
    ...
    hotelCards.forEach((card) => {
      ...
      if (head) {
        head.style.setProperty('background-color', '#ffffff')
      }
    ...
    if (budgetTotal) {
      ...
      el.style.setProperty('background-color', '#e3e9f9')
      el.style.setProperty('color', '#1a2332')
      el.style.setProperty('border', '1px solid #e2e8f0')
    ...
    const infoWindow = new AMap.InfoWindow({
      content: `
        <div style="padding: 10px;">
          ...
          <p style="margin: 4px 0; color: #4a7fd4;"><strong>第${attraction.dayIndex + 1}天 景点${attraction.attrIndex + 1}</strong></p>
        </div>
      `,
      offset: new AMap.Pixel(0, -30)
    })
